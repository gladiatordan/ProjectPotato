...
#cs
Summary:
Farms Sensali in Jaya Bluffs for Feathers and Feather Crests
Uses either Ritualist or any character with a Ritualist secondary profession

Requirements:
•	Factions
•	Ritualist (Or Rit 2nd Prof)
•	Seitung Harbor Outpost
•	Build: OACjAqiK5OQzH318bWOPbNTnJA
•	Staff with high energy (+15, +5 = +20 Energy) and 20% HSR
•	OR
•	Staff with High Energy 20% Enchant or Caster Set w/ 20% Enchant


;~ This was tested with:
;~ 		Full Shaman's Insignias
;~ 		+4 Channeling
;~ 		+1 Spawning
;~ 		+50 Health Rune
;~ 		+10 Health Rune
;~ 		+10 Health Rune
;~		Caster Axe +5 Energy 20% Enchant
;~		Shield +45 Enchant -2 While Enchant
;~		This build: OACjAqiK5OQzH318bWOPbNTnJA

For detailed information on this farm visit: https://wiki.guildwars.com/wiki/Guide_to_feather_farming
For detailed information on this farm build: visit: https://gwpvx.gamepedia.com/Build:Any/Rt_Feather_Farmer


v1.0 Updated 05.08.20(Restia Ashdoll)
;~Updated GWA2 to current public version

v1.1a Updated 08.08.20 (RiflemanX)
;~Updated GWA2 to current public version (Logicdoor/Zvend)
;~Corrected minor gui typo
;~Added function WaitMaploadingEx
;~Added Map Travel if in another outpost
;~Added function RndTravel
;~Randomized district travel
...
Global $Map_ID_Seitung_Harbor = 250
Global $Map_ID_Jaya_Bluffs = 196
Global $Seitung = 250 ; Seitungs Harbor
...
$Form1 = GUICreate("Refeather v1.1a", 618, 290, 100, 100)
$charactername = GUICtrlCreateInput("", 344, 24, 121, 21)
$startbutton = GUICtrlCreateButton("Start", 8, 224, 233, 57)
$cbxHideGW = GUICtrlCreateCheckbox("Dis. Graphics", 264, 80)
GUICtrlSetOnEvent($cbxHideGW, "gui_eventHandler")
$storegolds = GUICtrlCreateCheckbox("Store Golds", 264, 104)
$pickupweps = GUICtrlCreateCheckbox("Pickup bone and iron weps", 264, 56) ;new
$pickuparmors = GUICtrlCreateCheckbox("Pickup Armors", 350, 80) ;new
$storearmors = GUICtrlCreateCheckbox("Store Armors", 350, 104) ;new
...
$Label7 = GUICtrlCreateLabel("Feathered Crests got:", 24, 136, 105, 17, $SS_CENTER)
...
;$Label2 = GUICtrlCreateLabel("", 280, 46, 168, 17)
...
GUICtrlSetOnEvent($pickupweps, "UpdateConfig") ; new
GUICtrlSetOnEvent($pickuparmors, "UpdateConfig") ;new
GUICtrlSetOnEvent($storearmors, "UpdateConfig") ;new
...
While 1
	...
	If $boolRun Then
		Checkmap()
		If (IDorsellsomething() And $merch) Then SellAndBack()
		SwitchMode(0)
		SetUpFastWay()
		EnterArea()
		Farm()
...
Func SetUpFastWay()
	Update("Setting up resign")
	Zone()
	Move(10970, -13360)
	WaitMapLoadingEx(250)
	Sleep(1500)
	Return True
EndFunc
...
Func Checkmap()
	If GetMapID() <> $Map_ID_Seitung_Harbor Then
	    ;Out("Traveling: Seitung")
		RndTravel($Map_ID_Seitung_Harbor)
		WaitMapLoadingEx($Map_ID_Seitung_Harbor)
		Sleep(GetPing()+50)
	EndIf
EndFunc
...
Func WaitMapLoadingEx($iMap)
   ...
   EndIf
   If GetMapID() == $iMap Then
      Sleep(5000) ;~ Give time for full map load
...
Func RndTravel($aMapID)
	...
	Local $lRegion[11] = [2, 2, 2, 2, 0, -2, 1, 3, 4]
	Local $lLanguage[11] = [4, 5, 9, 10, 0, 0, 0, 0, 0]
	Local $Random = Random(0, $UseDistricts - 1, 1)
	MoveMap($aMapID, $lRegion[$Random], 0, $lLanguage[$Random])
	WaitMapLoading($aMapID, 15000)
EndFunc   ;==>RndTravel
...
Func IDAndSell()
	...
	If GetChecked($storegolds) Then
		...
		EndIf
	 EndIf
	; If GetChecked($storearmors) Then
	;	Update("Looking for armor items")
	;	If GUICtrlRead($usebag1) == $GUI_Checked Then
	;		ArmorIs(1, 20)
	;	EndIf
	;	If GUICtrlRead($usebag2) == $GUI_Checked Then
...
Func CanSell($aitem)
	...
	ElseIf $m = 18345 Then ;tokens
		Return False
   ElseIf $m = 941 Then ;Fur
		Return False
    ElseIf $m = 840 Then ;stolen supplies
		Return False
    ElseIf $m = 940 Then ;tanned
		Return False
   ElseIf $m = 925 Then ;cloth
		Return False
   ElseIf $m = 938 Then ;sapphire
		Return False
    ElseIf $m = 931 Then ;meye
		Return False
   ElseIf $m = 923 Then ;mfang
		Return False
	Else
		Return True
	EndIf
...
Func CanPickUp($aitem)
	...
	ElseIf $m = 21813 Then ;sparkler
		Return True
    ElseIf $m = 21812 Then ;sugary blue
		Return True
	ElseIf $m = 18345 Then ;tokens
		Return True
    ElseIf $m = 35124 Then ;brandy
		Return True
   ElseIf $m = 941 Then ;Fur
		Return True
    ElseIf $m = 840 Then ;stolen supplies
   ...
   ElseIf $m = 923 Then ;mfang
		Return True
   ;ElseIf $m = 1159 and ($r = $RARITY_Purple or $r = $RARITY_Blue) Then ;screstguard
	;	Return True
   ;ElseIf $m = 1156 and ($r = $RARITY_Purple or $r = $RARITY_Blue) Then ;sgarb
	;	Return True
   ;ElseIf $m = 1154 and ($r = $RARITY_Purple or $r = $RARITY_Blue) Then ;sgarb2
	;	Return True
   ;ElseIf $m = 1167 and ($r = $RARITY_Purple or $r = $RARITY_Blue) Then ;yplating
	;	Return True
   ;ElseIf $m = 697 and ($r = $RARITY_Purple or $r = $RARITY_Blue) Then ;yarmguard
	;	Return True
 ;  ElseIf $m = 1166 and ($r = $RARITY_Purple or $r = $RARITY_Blue) Then ;yrag
;		Return True
   ElseIf ($m = 1159 or $m = 1156 or $m = 1154 or $m = 1167 or $m = 697 or $m = 1166) and ($r = $RARITY_Purple or $r = $RARITY_Blue) and GetChecked($pickuparmors) Then ;armors with mods
		Return True
   ElseIf $m = 36683 Then ;pbeacon
		Return True
   ElseIf $m = 36681 Then ;dcake
		Return True
   ElseIf $m = 36682 Then ;itea
		Return True
   ElseIf ($m = 358 or $m = 360 or $m = 593 or $m = 763 or $m = 6537 or $m = 338 or $m = 797 or $m = 871 or $m = 872 or $m = 881 or $m = 162) and GetChecked($pickupweps) Then ;bone/iron weapons
;		Return True
;   ElseIf $m = 358 Then ;bstaff
;		Return True
;   ElseIf $m = 360 Then ;bstaff2
;		Return True
;   ElseIf $m = 593 Then ;bstaff3
;		Return True
;   ElseIf $m = 763 Then ;kamas
;		Return True
;   ElseIf $m = 6537 Then ;skey
;		Return True
;   ElseIf $m = 338 Then ;sshield
;		Return True
;   ElseIf $m = 797 Then ;sunqua blade
;		Return True
;   ElseIf $m = 871 Then ;spiked targe
;		Return True
;   ElseIf $m = 872 Then ;spiked targe
;		Return True
;   ElseIf $m = 881 Then ;bidol
;		Return True
;   ElseIf $m = 162 Then ;bidol
;		Return True
	Else
		Return False
	EndIf
