...
Func PickUpLoot($aMinSlots = 2)
   Local $lBlocked = 0
   Local $lMeX, $lMeY, $lAgentX, $lAgentY, $lAgentPtr
   Local $lMePtr = GetAgentPtr(-2)
   Local $lSlots = CountSlots()
   Local $lAgentArray = GetAgentPtrArrayEx(1, 0x400)
   For $i = 1 To $lAgentArray[0]
	  If GetIsDead($lMePtr) Then Return False ; died, cant pick up items dead
	  If GetMapLoading() <> 1 Then Return True ; not in explorable -> no items to pick up
	  If $lSlots < $aMinSlots Then Return False ; inventory is full
	  ...
	  If $mPickUpAll Or PickUpList($lItemPtr) Then
		 If $lDistance > 150 Then
			Do
			   Move_($lAgentX, $lAgentY)
			   Sleep(250)
			   $lBlocked += 1
			Until GetDistance($lAgentArray[$i], $lMePtr) <= 150 Or $lBlocked > 20
		 EndIf
		 $lTimer = TimerInit()
		 Do
			PickUpItem($lAgentID)
			Sleep(250)
		 Until GetAgentPtr($lAgentID) <> $lAgentArray[$i] Or TimerDiff($lTimer) > 3000
	  EndIf
   Next
EndFunc   ;==>PickUpLoot
...
Func GetNearestItemPtrTo($aAgent)
...
;~ Description: Upgrade Armor with Rune/Insignia
Func UpgradeArmor($aArmor, $aUpgrade)
   ...
   If $lUpgradeType = 0 Then Return ; neither rune nor insignia
   If $lUpgradeType = 2 Then $lUpgradeType = 0
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x62C] ; SalvageSessionID
   Local $lSessionID = MemoryReadPtr($mBasePointer, $lOffset)
   If IsArmor($lArmorType) = 0 Then Return False ; not an armor piece after all
   ...
   Sendpacket(0xC, 0x7A, $lSessionID[1], $lArmorID)
   PingSleep(125)
   Sendpacket(0x4, 0x7C)
EndFunc   ;==>UpgradeArmor
