;~ editable by Teqatle Version 3.7.1 30.05.2018
#include-once
#RequireAdmin
...
#Region Headers
;=QUEST=+
Global Const $HEADER_QUEST_ACCEPT						= 0x40	;Accepts a quest from the NPC
Global Const $HEADER_QUEST_REWARD						= 0x40	;Retrieves Quest reward from NPC
Global Const $HEADER_QUEST_ABANDON						= 0x11 	;Abandons the quest

;=HERO=+
Global Const $HEADER_HERO_AGGRESSION					= 0x16	;Sets the heroes aggression level
Global Const $HEADER_HERO_LOCK							= 0x17	;Locks the heroes target
Global Const $HEADER_HERO_TOGGLE_SKILL					= 0x1B	;Enables or disables the heroes skill
Global Const $HEADER_HERO_CLEAR_FLAG					= 0x1B	;Clears the heroes position flag
...
Global Const $HEADER_HENCHMAN_ADD						= 0xA4	;Adds henchman to party
Global Const $HEADER_PARTY_LEAVE						= 0xA7	;Leaves the party
Global Const $HEADER_HENCHMAN_KICK						= 0xAD	;Kicks a henchman from party
Global Const $HEADER_INVITE_TARGET						= 0xA3	;Invite target player to party
Global Const $HEADER_INVITE_CANCEL						= 0xA1	;Cancel invitation of player
Global Const $HEADER_INVITE_ACCEPT						= 0xA0-1	;Accept invitation to party

;=TARGET (Enemies or NPC)=+
Global Const $HEADER_CALL_TARGET						= 0x27	;Calls the target without attacking (Ctrl+Shift+Space)
Global Const $HEADER_ATTACK_AGENT						= 0x2B	;Attacks agent (Space IIRC)
Global Const $HEADER_CANCEL_ACTION						= 0x2D	;Cancels the current action
Global Const $HEADER_GO_PLAYER							= 0x38	;Follows the agent/npc. Ctrl+Click triggers "I am following Person" in chat
Global Const $HEADER_NPC_TALK							= 0x3E	;talks/goes to npc
Global Const $HEADER_SIGNPOST_RUN						= 0x56	;Runs to signpost

;=DROP=+
Global Const $HEADER_ITEM_DROP							= 0x31	;Drops item from inventory to ground
Global Const $HEADER_GOLD_DROP							= 0x34	;Drops gold from inventory to ground

;=BUFFS=+
Global Const $HEADER_STOP_MAINTAIN_ENCH					= 0x2E	;Drops buff, cancel enchantmant, whatever you call it

;=ITEMS= +
Global Const $HEADER_ITEM_EQUIP							= 0x35	;Equips item from inventory/chest/no idea
Global Const $HEADER_ITEM_PICKUP						= 0x44	;Picks up an item from ground
Global Const $HEADER_ITEM_DESTROY						= 0x6B ;Destroys the item
Global Const $HEADER_ITEM_ID							= 0x70	;Identifies item in inventory
Global Const $HEADER_ITEM_MOVE							= 0x76	;Moves item in inventory
Global Const $HEADER_ITEMS_ACCEPT_UNCLAIMED				= 0x77	;Accepts ITEMS not picked up in missions
Global Const $HEADER_ITEM_MOVE_EX						= 0x77	;Moves an item, with amount to be moved.
Global Const $HEADER_SALVAGE_MATS						= 0x7E	;Salvages materials from item
Global Const $HEADER_SALVAGE_MODS						= 0x7F	;Salvages mods from item
Global Const $HEADER_ITEM_USE							= 0x82	;Uses item from inventory/chest
...
;=TRADE=+
Global Const $HEADER_TRADE_PLAYER						= 0x4E	;Send trade request to player
Global Const $HEADER_TRADE_OFFER_ITEM					= 0x01	;Add item to trade window
Global Const $HEADER_TRADE_SUBMIT_OFFER					= 0x02	;Submit offer
Global Const $HEADER_TRADE_CHANGE_OFFER					= 0x04	;Change offer
Global Const $HEADER_TRADE_CANCEL						= 0x00	;Cancel trade
Global Const $HEADER_TRADE_ACCEPT						= 0x06	;Accept trade

;=TRAVEL=
Global Const $HEADER_MAP_TRAVEL							= 0xB6	;Travels to outpost via worldmap
Global Const $HEADER_GUILDHALL_TRAVEL					= 0xB5	;Travels to guild hall
Global Const $HEADER_GUILDHALL_LEAVE					= 0xB7	;Leaves Guildhall
Global Const $HEADER_OPEN_GB_WINDOW 					= 0x9D  ;îòêðûòü îêíî áèòâà ãèëüäèé
Global Const $HEADER_CLOSE_GB_WINDOW 					= 0x9E  ;çàêðûòî îêíî áèòâà ãèëüäèé
Global Const $HEADER_START_RATING_GVG					= 0xA9

;=FACTION=+
Global Const $HEADER_FACTION_DONATE						= 0x3A	;Donates kurzick/luxon faction to ally

;=TITLE=
Global Const $HEADER_TITLE_DISPLAY						= 0x5A	;Displays title (from Gigis Vaettir Bot)
Global Const $HEADER_TITLE_CLEAR						= 0x5B	;Hides title (from Gigis Vaettir Bot)

;=DIALOG=+
Global Const $HEADER_DIALOG								= 0x40	;Sends a dialog to NPC
Global Const $HEADER_CINEMATIC_SKIP						= 0x67	;Skips the cinematic

;=SKILL / BUILD=+
Global Const $HEADER_SET_SKILLBAR_SKILL					= 0x60	;Changes a skill on the skillbar
Global Const $HEADER_LOAD_SKILLBAR						= 0x61	;Loads a complete build
Global Const $HEADER_CHANGE_SECONDARY					= 0x46	;Changes Secondary class (from Build window, not class changer)
...
Global Const $HEADER_SET_ATTRIBUTES						= 0x0D	;hidden in init stuff like sendchat

;=CHEST=+
Global Const $HEADER_CHEST_OPEN							= 0x58	;Opens a chest (with key AFAIK)
Global Const $HEADER_CHANGE_GOLD						= 0x80	;Moves Gold (from chest to inventory, and otherway around IIRC)

;=MISSION=+
Global Const $HEADER_MODE_SWITCH						= 0xA0	;Toggles hard- and normal mode
Global Const $HEADER_MISSION_ENTER						= 0xAA	;Enter a mission/challenge
Global Const $HEADER_MISSION_FOREIGN_ENTER				= 0xA8	;Enters a foreign mission/challenge (no idea honestly)
Global Const $HEADER_OUTPOST_RETURN 					= 0xAC	;Returns to outpost after /resign
...
Local $mReadChat
Local $mMouseMoveFunctionX
Local $mMouseMoveFunctionY
...
Func DelCrashLogs()
   FileDelete(@MyDocumentsDir & '\GUILD WARS\ArenaNet.log')
   FileDelete(@MyDocumentsDir & '\GUILD WARS\Crash.dmp')
   Local Const $lList0 = _FileListToArrayRec('C:\1\', 'ArenaNet.log', 1, -1)
   If Not @error Then
	  If UBound($lList0) >= 2 Then
		 For $i In $lList0
			FileDelete('C:\1\' & $i)
		 Next
	  EndIf
   EndIf
   Local Const $lList1 = _FileListToArrayRec('A:\', 'ArenaNet.log', 1, -1)
	If Not @error Then
		If UBound($lList1) >= 2 Then
			For $i In $lList1
				FileDelete('A:\' & $i)
			Next
		EndIf
	 EndIf
   Local Const $lList2 = _FileListToArrayRec('D:\', 'ArenaNet.log', 1, -1)
	If Not @error Then
		If UBound($lList2) >= 2 Then
			For $i In $lList2
				FileDelete('D:\' & $i)
			Next
		EndIf
...
;~ 	$mBuildNumber = MemoryRead(GetScannedAddress('ScanBuildNumber', 0x54)) old
	...
	$mReadChat = MemoryRead(GetScannedAddress('ScanReadChatFunction', 2))
	$mMouseMoveFunctionX = MemoryRead(GetScannedAddress('ScanMouseMoveFunction', 2))
	$mMouseMoveFunctionY = MemoryRead(GetScannedAddress('ScanMouseMoveFunction', 10))
...
;~ 	Local $lInitializ = BinaryToString('0x5C57696E646F777320475C4775696C64576172732E646C6C')
;~ 	If FileExists(@ProgramFilesDir & $lInitializ) Then
;~ 	Else
;~ 		Exit
;~ 	EndIf
...
Func Scan()
	...
	_('ScanBuildNumber:')
	AddPattern('558BEC83EC4053568BD9') ;AddPattern('8D8500FCFFFF8D') old
    _('ScanChangeStatusFunction:')
	AddPattern('568BF183FE047C146818020000') ;568BF183FE047C146811020000
	 _('ScanReadChatFunction:')
	AddPattern('A128B6EB00')
	_('ScanMouseMoveFunction:')
	AddPattern('A3706CA3008B4204')
	_('ScanDialogID:')
	AddPattern('8B402C8B808006000085C074138B800C010000F7D0A81074078BCEE808')
...
Func CraftItemExmod($aModelID, $aQuantity, $aGold, ByRef $aMatsArray)
	Local $pSrcItem = GetInventoryItemPtrByModelId($aMatsArray[0][0])
	If ((Not $pSrcItem) Or (MemoryRead($pSrcItem + 0x4B) < $aMatsArray[0][1])) Then Return 0
	Local $pDstItem = MemoryRead(GetMerchantItemPtrByModelId($aModelID))
	If (Not $pDstItem) Then Return 0
	...
	Local $lMatsArraySize = UBound($aMatsArray) - 1
	;  For $i = $lMatsArraySize To 0 Step -1
	$lCheckQuantity = CountItemInBagsByModelID($aMatsArray[0][0])
	;	  If $aMatsArray[$i][1] * $aQuantity > $lCheckQuantity Then  ; not enough mats in inventory
	;		 Return SetExtended($aMatsArray[$i][1] * $aQuantity - $lCheckQuantity, $aMatsArray[$i][0]) ; amount of missing mats in @extended
	;	  EndIf
	;  Next
	...
	;   $CraftMatsType &= ';dword'
	;Next
	$CraftMatsBuffer = DllStructCreate('dword;dword')
	$CraftMatsPointer = DllStructGetPtr($CraftMatsBuffer)
	For $i = 1 To $lMatCount
		$lSize = StringInStr($lMatString, ';')
		DllStructSetData($CraftMatsBuffer, $i, StringLeft($lMatString, $lSize - 1))
	...
	Next
	Local $lMemSize = $lMatCount * 4
	Local $lBufferMemory = DllCall($mKernelHandle, 'ptr', 'VirtualAllocEx', 'handle', $mGWProcHandle, 'ptr', 0, 'ulong_ptr', $lMemSize, 'dword', 0x200, 'dword', 0x10)
	If $lBufferMemory = 0 Then Return 0 ; couldnt allocate enough memory
	Local $lBuffer = DllCall($mKernelHandle, 'int', 'WriteProcessMemory', 'int', $mGWProcHandle, 'int', $lBufferMemory[0], 'ptr', $CraftMatsPointer, 'int', $lMemSize, 'int', '')
	If $lBuffer = 0 Then Return
	DllStructSetData($mCraftItemEx, 2, $aQuantity)
...
Func ChangeGold($aCharacter, $aStorage)
	Return SendPacket(0xC, $HEADER_GOLD_MOVE, $aCharacter, $aStorage) ;0x75
EndFunc   ;==>ChangeGold
#EndRegion Item
...
Func getnearestnpctocoord($aName)
...
Func InviteGuild($charName)
	If GetAgentExists(-2) Then
        DllStructSetData($mInviteGuild, 1, GetValue('CommandPacketSend'))
        DllStructSetData($mInviteGuild, 2, 0x4C)
        DllStructSetData($mInviteGuild, 3, 0xB7)
...
;~ Func GetHealthCheck() ;  Don't Continue on while anyone has low health, Returns TRUE if all have good HP
...
;~ 			If DllStructGetData($lAgent, 'HP') < 0.40 Then
;~ 			   Update("Waiting for party heal")
;~ 			   $MoveX = DllStructGetData($lAgent, 'X')
;~ 			   $MoveY = DllStructGetData($lAgent, 'Y')
;~ 			   MoveEx($MoveX, $MoveY)
;~ 			   RndSleep(1000)
;~ 			   Return False
;~ 			EndIf
...
Func StartRatingGvG()
	If GetAgentExists(-2) Then
	    SendPacket(0x4, $HEADER_OPEN_GB_WINDOW) ; îòêðûòü îêíî áèòâà ãèëüäèé
;~ 	    SendPacket(0x4, $HEADER_CLOSE_GB_WINDOW) ;çàêðûòî îêíî áèòâà ãèëüäèé
        SendPacket(0x88, $HEADER_START_RATING_GVG, 3) ;ðåéòèíãîâûé àâòîìàò
	Return True
	Else
		Return False
	EndIf
...
Func TradeBalthazarX()
	...
	$MyName = GetCharname()
	If GetMapID() == 330 Then
		Sleep(Random(750, 3000, 1))
		$npc = GetNearestNPCToCoords(1164, -4993) ;$npc = getnearestnpctocoord("Tolkano [Tournament]")
		RndSleep(500)
        GoToNPC($npc)
		If GetBalthazarFaction() >= 5000 Then
			RndSleep(500)
