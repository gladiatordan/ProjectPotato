...
If $FirstRun Then
   ...
   If GetMapID() <> 349 Then travelTo(349)
    ...
    upd("Loading Skills")
	;ClearAttributes()
    LoadSkillTemplate("OgcTcZ/85p4ozOgsyGoLD3JcAA")
...
upd("Killing")
If GetIsLiving(-2) Then UseSkillEx(2,GetNearestAgentToCoords(-6036, 17064))

upd("Killing")
If GetIsLiving(-2) Then UseSkillEx(6,GetNearestAgentToCoords(-6036, 17064))
...
Do
	Sleep(250)
Until GetDistance(-2,GetNearestAgentToCoords(-6036, 17064)) < 160 or GetIsDead(-2)
If GetIsLiving(-2) Then UseSkillEx(6,-2)
If GetIsDead(-2) Then OmgTheyKilledKennyYouBastards()
...
#Region Inventory
Func Inventory($aBags = 6)
    upd("Travelling to Guild Hall")
    TravelGH()
	upd("Going to Xunlai Chest")
    $npc = GetNearestNPCToCoords(-4600, -1479)
	GoToNPC ($npc)
	sleep(500)
	upd("Going to Merchant")
	$npc2 = GetNearestNPCToCoords(-4061, -1132)
    GoToNPC ($npc2)
...
Switch $t
   ...
   Case Else
	   If  $m = 22751  or $m = 146 or $m = 934 or $m = 956 Then
