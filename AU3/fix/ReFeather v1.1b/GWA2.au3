...
Func GetItemIDfromModelID($aModelID)
	For $i = 1 To 4
...
Func GetItemBySlot2($aBag, $aSlot)
	Local $lBag
...
Func WaitMapLoading2($aMapID = 0, $aDeadlock = 10000)
;~ 	Waits $aDeadlock for load to start, and $aDeadLock for agent to load after map is loaded.
	Local $lMapLoading
	Local $lDeadlock = TimerInit()
   ...
   Out("InitMapLoad()")
	Do
		Sleep(200)
		$lMapLoading = GetMapLoading()
		Out("$lMapLoading = GetMapLoading()")
		If $lMapLoading == 2 Then $lDeadlock = TimerInit()
		If TimerDiff($lDeadlock) > $aDeadlock And $aDeadlock > 0 Then Return False
		; Out("If TimerDiff($lDeadlock) > $aDeadlock And $aDeadlock > 0 Then Return False)")
	Until $lMapLoading <> 2 And GetMapIsLoaded() And (GetMapID() = $aMapID Or $aMapID = 0)
