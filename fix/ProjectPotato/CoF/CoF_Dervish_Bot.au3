...
#Region GUI
$Gui = GUICreate("Derv CoF 2019", 299, 174, -1, -1)
...
#Region Loops
Out("Ready.")
Out("CoF Dervish Bot Updated By Kai194.")
...
Func CanPickUp($lItem)
   ...
   If $ModelID == 921 Then	; Bones
	  ...
	  Return True
   EndIf
   If $lType == 11 Then Return True
   If $lType == 20 And GetGoldCharacter() < 99000 Then Return True
   If $lType == 9 then return true
   If $lType == 17 then return true
   If $lType == 18 then return true
   If $lType == 30 then return true
   If $lType == 31 then return true
   Return False
EndFunc
