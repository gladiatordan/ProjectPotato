...
Func Setup()
   If _IsChecked($FarmKebob) OR $FarmMode = 1 Then
	  If GetMapID() <> 425 Then
		 Out("Travelling to Rilohn.")
		 RndTravel(425)
	  EndIf
	  Out("Loading skillbar.")
	  LeaveGroup()
	  SwitchMode(0)
	  LoadSkillTemplate("Ogek8Np5Kzmj59brdbu731L7FBC")
	  Out("Adding Koss")
	  KickAllHeroes()
	  Do
		 AddHero($Hero_Koss)
		 RndSleep(100)
	  Until GetHeroCount() = 1
   ...
   If _IsChecked($FarmSalad) OR $FarmMode = 2 Then
	  If GetMapID() <> 449 Then
		 Out("Travelling to Kamadan.")
		 RndTravel(449)
	  EndIf
	  Out("Loading skillbar.")
	  LeaveGroup()
	  SwitchMode(0)
	  LoadSkillTemplate("Ogakgwp5ayOERD3HAAAAAA71dpqI")
	  ChangeWeaponSet($WEAPON_SLOT_SHIELD)
	  RndSleep(500)
	  SetUpFastWay()
   EndIf
   If _IsChecked($FarmSoup) OR $FarmMode = 3 Then
	  ...
	  EndIf
	  Out("Loading skillbar.")
	  LoadSkillTemplate("Ogek8Np5Kzmk513GBWzlqIuz+F7F")
	  LeaveGroup()
	  SwitchMode(0)
	  ChangeWeaponSet($WEAPON_SLOT_SHIELD)
	  RndSleep(500)
...
Func KillNextIboga()
		...
		If NOT DllStructGetData($NextIboga, "PlayerNumber") = 4384 Then
			Do
				$NextIboga = GetAgentByID(-1)
				Sleep(100)
				TargetNextEnemy()
			Until DllStructGetData($NextIboga, "PlayerNumber") = 4384
...
Func GetIbogaInRange($aAgent = -2, $aRange = 4500)
		...
		For $i = 1 To $lAgentArray[0]
				...
				$lDistance = GetDistance($lAgent)
				If $lDistance > $aRange Then ContinueLoop
				If DllStructGetData($lAgent, 'PlayerNumber') == 4384 Then $lCount += 1	;Ibogas = 4384
		Next
		Return $lCount
EndFunc
