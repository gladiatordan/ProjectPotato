...
Func CanUseSkill($aSkillSlot, $aEnergy = 0, $aSoftCounter = 0)
   ...
   If GetSkillbarSkillRecharge($aSkillSlot) = 0 Then
	  If GetSkillbarSkillAdrenaline($aSkillSlot) < $mSkillbarCacheArray[$aSkillSlot][7] Then Return False
...
Func RezParty($RezSkill)
   Local $lAgentX, $lAgentY, $lMeX, $lMeY
   Local $Me = GetAgentPtr(-2)
   Local $lBlocked = 0
   ...
   Do
	  ...
	  If Not GetIsMoving($Me) Then
		 ...
		 Move($lAgentX, $lAgentY, 250)
		 Sleep(200)
		 If Mod($lBlocked, 5) = 0 And Not GetIsMoving($Me) Then
			$theta = Random(0, 360)
			UpdateAgentPosByPtr($Me, $lMeX, $lMeY)
...
Func SmartCast()
   ...
   For $r = 8 To 1 Step -1 ; Skill priority set in Ascending order.
	  ...
	  If $SkillAdrenalineReq[$i] = 0 Then
		 If GetSkillbarSkillRecharge($i) Or Not CanUseSkill($i, $mSkillbarCacheEnergyReq[$i]) Then ContinueLoop
