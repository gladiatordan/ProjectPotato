...
Func GetSkillbarSkillRecharge($aSkillSlot, $aHeroNumber = 0, $aPtr = 0)
   If $aPtr = 0 Then
	  Local $lPtr = GetSkillbarPtr($aHeroNumber)
   Else
	  Local $lPtr = $aPtr
   EndIf
   $aSkillSlot -= 1
   $lTimestamp = MemoryRead($lPtr + 12 + $aSkillSlot * 20, 'dword')
...
Func GetSkillbarSkillAdrenaline($aSkillSlot, $aHeroNumber = 0)
   $aSkillSlot -= 1
   Return MemoryRead(GetSkillbarPtr($aHeroNumber) + 4 + $aSkillSlot * 20, 'long')
...
Func GetSkillbarPtr($aHeroNumber = 0)
   ...
   Local $lOffset[5] = [0, 0x18, 0x2C, 0x68C]
...
Func IsRecharged($aSkillSlot)
   Return GetSkillbarSkillRecharge($aSkillSlot) = 0
EndFunc   ;==>IsRecharged
...
Func GetSkillbarSkillID($aSkillSlot, $aHeroNumber = 0)
   Local $lPtr = GetSkillbarPtr($aHeroNumber)
   $aSkillSlot -= 1
   Return MemoryRead($lPtr + 16 + $aSkillSlot * 20, 'dword')
...
Func UseSkillEx($aSkillSlot, $aTarget = -2, $aTimeout = 3000, $aMeleeSkill = False)
   ...
   Do
	  ...
	  If GetEnergy($lMePtr) < MemoryRead($lSkillPtr + 53, 'byte') Then Return ; not enough energy
...
Func UseSkillByID($aSkillID, $aAgentID = 0)
   For $i = 0 to 7
	  If GetSkillbarSkillID($i) = $aSkillID Then
		 Return UseSkillEx($i+1, $aAgentID)
	  EndIf
   Next
...
Func UseRezSkillEx($aSkillSlot, $aTarget, $aTimeOut = 7000)
   If IsPtr($aTarget) <> 0 Then
   ...
   Do
	  ...
	  Death()
	  If TimerDiff($lDeadlock) > $aTimeOut Then Return
   Until GetSkillbarSkillRecharge($aSkillSlot) <> 0 Or Not GetIsDead(-1)
   If GetIsCasting() Then CancelAction()
EndFunc   ;==>UseRezSkillEx

;~ Description: Remove hex with updated check.
Func RemoveHexSkillEx($aSkillSlot, $aTarget, $aTimeOut = 3500)
   If IsPtr($aTarget) <> 0 Then
	  $lTargetID = MemoryRead($aTarget + 44, 'long')
   ElseIf IsDllStruct($aTarget) <> 0 Then
	  $lTargetID = DllStructGetData($aTarget, 'ID')
   ...
   Do
	  ...
	  Death()
	  If TimerDiff($lDeadlock) > $aTimeOut Then Return
   Until GetSkillbarSkillRecharge($aSkillSlot) <> 0 Or Not GetHasHex($aTarget) Or GetIsDead(-1)
   If GetIsCasting() Then CancelAction()
...
Func GetBuffCount($aHeroNumber = 0)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
...
Func DropAllBonds($aHeroNumber = 0)
   Local $lBuffCount = GetBuffCount($aHeroNumber)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
   Local $lCount = MemoryReadPtr($mBasePointer, $lOffset)
   ReDim $lOffset[5]
...
Func DropAllBuffs($aHeroNumber = 0)
   Local $lBuffCount = GetBuffCount($aHeroNumber)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
   Local $lCount = MemoryReadPtr($mBasePointer, $lOffset)
   ReDim $lOffset[5]
...
Func GetBuffPtr($aBuffNumber, $aHeroNumber = 0)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
   Local $lCount = MemoryReadPtr($mBasePointer, $lOffset)
   ReDim $lOffset[5]
   $lOffset[3] = 0x4A4
...
Func HasEffect($aEffectSkillID, $aHeroNumber = 0)
   Return GetSkillEffectPtr($aEffectSkillID, $aHeroNumber) <> 0
EndFunc   ;==>HasEffect
...
Func GetSkillEffectPtr($aSkillID, $aHeroNumber = 0)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
   Local $lCount = MemoryReadPtr($mBasePointer, $lOffset)
   ReDim $lOffset[5]
   $lOffset[3] = 0x4A4
...
Func GetSkillEffectPtrByEffectnumber($aEffectNumber, $aHeroNumber = 0)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
   Local $lCount = MemoryReadPtr($mBasePointer, $lOffset)
   ReDim $lOffset[5]
   $lOffset[3] = 0x4A4
...
Func GetEffectsPtr($aSkillID = 0, $aHeroNumber = 0)
   Local $lEffectCount, $lEffectStructAddress, $lBuffer
   Local $lReturnArray[2] = [0]
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x4AC]
   Local $lCount = MemoryReadPtr($mBasePointer, $lOffset)
   ReDim $lOffset[5]
   $lOffset[3] = 0x4A4
	  ...
	  If $lBuffer[1] = GetHeroID($aHeroNumber) Then
		 ...
		 If $aSkillID = 0 Then
			...
			For $i = 0 To $lEffectCount[1] - 1
			   $lReturnArray[$i + 1] = Ptr($lEffectStructAddress[0] + 24 * $i)
			Next
		 Else
			For $i = 0 To $lEffectCount[1] - 1
			   $lReturn = $lEffectStructAddress[0] + 24 * $i
			   If MemoryRead($lReturn, 'long') = $aSkillID Then $lReturnArray[1] = $lReturn
			Next
		 EndIf
	  EndIf
   Next
...
Func GetMorale($aHeroNumber = 0)
   Local $lAgentID = GetHeroID($aHeroNumber)
   Local $lOffset[4] = [0, 0x18, 0x2C, 0x5D4]
   Local $lIndex = MemoryReadPtr($mBasePointer, $lOffset)
   Local $lOffset[6] = [0, 0x18, 0x2C, 0x5C8, 8 + 0xC * BitAND($lAgentID, $lIndex[1]), 0x18]
