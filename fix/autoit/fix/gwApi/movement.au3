...
Func MoveTo($aX, $aY, $aRandom = 50)
   Local $lBlocked = 0
   Local $lMe, $lMoveX, $lMoveY, $lMeX, $lMeY
   ...
   Do
	  Sleep(100)
	  $lMe = GetAgentPtr(-2)
	  If MemoryRead($lMe + 304, 'float') <= 0 Then Return False
	  $lMapLoadingOld = $lMapLoading
	  $lMapLoading = GetMapLoading()
	  If $lMapLoading <> $lMapLoadingOld Then Return False
	  UpdateAgentMoveByPtr($lMe, $lMoveX, $lMoveY)
...
Func MoveToAlert($aX, $aY, $aRandom = 0)
   ...
   Do
	  Sleep(100)
	  $lMe = GetAgentPtr(-2)
	  If MemoryRead($lMe + 304, 'float') <= 0 Then ExitLoop
	  $lMapLoadingOld = $lMapLoading
	  $lMapLoading = GetMapLoading()
	  If $lMapLoading <> $lMapLoadingOld Then ExitLoop
	  If GetNumberOfFoesInRangeOfAgent(-2, 5000) > 0 Then
		 Do
			UpdateWorld()
...
Func MoveToLeader($aDistance = 1200)
...
Func MoveAggroing($MoveToX, $MoveToY, $SleepTime = 3600000)
   ...
   Do ;;;;;;;;;; Leader LOOP
	  ...
	  If $mLowestEnemy = 0 And Not GetIsMoving(-2) Then
		 $lBlocked += 1
	  ...
	  OutpostCheck()
	  Sleep(100)
	  If ComputeDistance($MoveToX, $MoveToY, $MyLocation[0], $MyLocation[1]) < 200 And $EnemyAttacker = 0 Then Return
...
Func GetHealthCheck()
   Local $lAgentArray = GetAgentPtrArrayEx(3, 0xDB, 1, True)
   Local $MoveX, $MoveY
   If $mClosestEnemy <> 0 Then Return True ; Return if someone to fight
   For $i = 1 To $lAgentArray[0]
	  If MemoryRead($lAgentArray[$i] + 304, 'float') < 0.40 Then
		 Update("Waiting for party heal")
		 UpdateAgentPosByPtr($lAgentArray[$i], $MoveX, $MoveY)
		 MoveEx($MoveX, $MoveY)
		 RndSleep(1000)
		 Return False
	  EndIf
...
Func MoveIfHurt()
   ...
   If GetIsDead() Then Return False
   If $AOEDanger And TimerDiff($AOEDangerTimer) < $AOEDangerDuration * 1000 Then
	  UpdateAgentPosByPtr($lMePtr, $lX, $lY)
	  ...
	  If $DistanceToAOEZone <= $AOEDangerRange + 50 Then
		 ...
		 $SafeSpot = GetClosestSafeZone($AOEDangerXLocation, $AOEDangerYLocation, 500)
		 $HurtTimer = TimerInit()
		 Move($SafeSpot[0], $SafeSpot[1], 0)
		 Sleep(200)
		 Do
			If Not GetIsMoving($lMePtr) Then $lBlocked += 1
			Sleep(100)
			UpdateAgentPosByPtr($lMePtr, $lX, $lY)
   ...
   ElseIf $AOEDanger And TimerDiff($AOEDangerTimer) > $AOEDangerDuration * 1000 Then
	  $AOEDanger = False
   EndIf
   If GetHealth() < $CurrentHP And Not $AOEDanger Then
	  If TimerDiff($HurtTimer) > 1000 And Not GetHasDegenHex(-2) Then
		 $theta = Random(0, 360)
		 $HurtTimer = TimerInit()
		 UpdateAgentPosByPtr(GetAgentPtr($mHighestAlly), $lX, $lY)
...
Func GoNPC($aAgent)
   If IsPtr($aAgent) <> 0 Then
	  Return SendPacket(0xC, 0x33, MemoryRead($aAgent + 44, 'long'))
   ElseIf IsDllStruct($aAgent) <> 0 Then
	  Return SendPacket(0xC, 0x33, DllStructGetData($aAgent, 'ID'))
   Else
	  Return SendPacket(0xC, 0x33, ConvertID($aAgent))
   EndIf
EndFunc   ;==>GoNPC
...
Func GoToNPC($aAgent)
   ...
   Do
	  Sleep(100)
	  If MemoryRead($lMePtr + 304, 'float') <= 0 Then Return False
	  $lMapLoadingOld = $lMapLoading
	  $lMapLoading = GetMapLoading()
	  If $lMapLoading <> $lMapLoadingOld Then Return False
	  UpdateAgentMoveByPtr($lMePtr, $lMoveX, $lMoveY)
	  If $lMoveX = 0 And $lMoveY = 0 Then
		 $lBlocked += 1
		 UpdateAgentPosByPtr($lMePtr, $lMeX, $lMeY)
		 If ComputeDistance($lMeX, $lMeY, $lAgentX, $lAgentY) < 250 Then
			Sleep(1000 + GetPing())
			Return True
		 EndIf
		 Move($lAgentX, $lAgentY, 100)
...
Func GoFindNPC($aAgent, $aRange = 3000)
   If IsPtr($aAgent) <> 0 Then
	  Local $lPtr = $aAgent
   ElseIf IsDllStruct($aAgent) <> 0 Then
   ...
   Do
	  ...
	  EndIf
	  UpdateAgentPosByPtr($lMePtr, $lMeX, $lMeY)
   Until ComputeDistance($lX, $lY, $lMeX, $lMeY) < $aRange Or $lBlocked > 14
   Sleep(250)
   Return 1
EndFunc   ;==>GoFindNPC
...
Func GoToSignpost($aAgent)
   ...
   Do
	  Sleep(100)
	  If MemoryRead($lMePtr + 304, 'float') <= 0 Then ExitLoop
	  $lMapLoadingOld = $lMapLoading
	  $lMapLoading = GetMapLoading()
	  If $lMapLoading <> $lMapLoadingOld Then ExitLoop
	  UpdateAgentMoveByPtr($lMePtr, $lMoveX, $lMoveY)
...
Func GoLuxons($XLocation, $YLocation)
   If GetLuxonFaction() > 12000 Then
	  ...
	  GoToNPC(GetNearestAgentPtrToCoords($XLocation, $YLocation))
	  Sleep(1000)
	  Do
...
Func GetInUnderworld()
   ...
   If ComputeDistance($lX, $lY, -4196, 19781) > 200 Then
	  Do
		 If Not GetIsMoving($lMePtr) Then
			MoveTo($lX, $lY, 300)
			PingSleep(500)
			MoveTo(-4196, 19781)
		 EndIf
...
Func GoToNPCbyPlayernumber($aPlayernumber)
   $lAgentArray = GetAgentPtrArrayEx(1)
   For $i = 1 To $lAgentArray[0]
